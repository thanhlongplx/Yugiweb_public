...
const UserDetail = () => {
  ...
  const [friendRequests, setFriendRequests] = useState([]); // Danh sách yêu cầu kết bạn
  ...
  useEffect(() => {
    ...
    const fetchFriends = async () => {
      try {
        const response = await fetch(
          `http://localhost:5000/api/friends/${sessionStorage.getItem(
            "userUsername"
          )}`
        );
        if (!response.ok) {
    ...
    const fetchFriendRequests = async () => {
      try {
        const response = await fetch(
          `http://localhost:5000/api/friend-requests/${sessionStorage.getItem(
            "userUsername"
          )}`
        );
        if (!response.ok) {
        ...
        const data = await response.json();
        setFriendRequests(data.map((request) => request.username)); // Lấy username từ danh sách yêu cầu kết bạn
      } catch (err) {
        setError(err.message);
      }
    };
...
const hasSentRequest = friendRequests.includes(username); // Kiểm tra nếu đã gửi yêu cầu kết bạn
const hasReceivedRequest = friendRequests.includes(sessionStorage.getItem("userUsername")); // Kiểm tra nếu đã nhận yêu cầu từ user đang xem

// Kiểm tra xem có thể gửi yêu cầu hay không
const canSendRequest = !isFriend && !hasSentRequest && !hasReceivedRequest && !(hasSentRequest && hasReceivedRequest);

  return (
    <div className="bg-dark">
      <div className="container pb-5">
        <div className="card shadow-sm bg-dark border-0 mb-4">
          <div className="card-body bg-dark shadow text-white">
